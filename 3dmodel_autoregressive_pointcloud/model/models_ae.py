...
def fps(data, number, data_3d):
    ...
    '''
    B, N, D = data.size()
    data = data.contiguous().view(B, N, D)
    fps_idx = pointnet2_utils.furthest_point_sample(data, number) 
...
class AutoEncoder(nn.Module):
    ...
    def encode(self, pc):
        ...
        ###### fps
        flattened = pc.contiguous().view(B*N, D)
        #print('flattened.shape: ',flattened.shape)
        '''flattened.shape:  torch.Size([2048, 3])'''
        batch = torch.arange(B).to(pc.device)
        #print('batch.shape: ',batch.shape)
        '''batch.shape:  torch.Size([1])'''
        batch = torch.repeat_interleave(batch, N)
        #print('batch1.shape: ',batch.shape)
        '''batch1.shape:  torch.Size([2048])'''
        pos = flattened
        ...
        # '''idx.shape:  torch.Size([512])'''
        # sampled_pc = pos[idx]
        # #print('sampled_pc.shape: ',sampled_pc.shape)
        # '''sampled_pc.shape:  torch.Size([512, 3])'''
        # sampled_pc = sampled_pc.view(B, -1, 3)
        # #print('sampled_pc2.shape: ',sampled_pc.shape)
        # '''sampled_pc2.shape:  torch.Size([1, 512, 3])'''
        # ######
        
        sampled_pc = fps(pc,512,pc)
        print('sampled_pc.shape: ',sampled_pc.shape)
        sampled_pc_embeddings = self.point_embed(sampled_pc)
        print('sampled_pc_embeddings.shape: ',sampled_pc_embeddings.shape)
        '''sampled_pc_embeddings.shape:  torch.Size([1, 512, 512])'''
        


        pc_embeddings = self.point_embed(pc)
        #print('pc_embeddings.shape: ',pc_embeddings.shape)
        '''pc_embeddings.shape:  torch.Size([1, 2048, 512])'''
...
if __name__ =='__main__':
    ...
    cad_data = torch.rand(1, 2048, 3).to(device)
