...
class Views2Points(nn.Module):
    ...
    def __init__(self,config):
        ...
        N = 1728
        M = 512
        self.attention_encode = AutoEncoder(
            depth=8,
            dim=dim,
            queries_dim=dim,
            output_dim = 1,
    ...
    def forward_encoder(self, cad_data, command, args):
        ...
        neighborhood, center = self.group_divider(cad_data,cad_data)
        print('neighborhood.shape:',neighborhood.shape)
        #print('center.shape:',center.shape)
        ...
        z = x_full+pos_full
        return z
    def attention_encoder(self,top,front,side,cad_data):
        # side_feature = self.img_feature(side)
        # front_feature = self.img_feature(front)
        # top_feature = self.img_feature(top)
        # # print('side_feature.shape: ',side_feature.shape)
        ...
        x = self.attention_encode.encode(cad_data)
    ...
    def forward_decoder(self,z,command, args):
        ...
        # image_feat = F.normalize(z[:,0,:],dim=-1)
        # text_feat = F.normalize(text[:,0,:],dim=-1)
        # with torch.no_grad():
        ...
        return res,loss_ita
    def forward(self ,top_pic,front_pic,side_pic,cad_data, command, args):
        
        #z = self.forward_encoder(cad_data, command, args)
        z = self.attention_encoder(top_pic,front_pic,side_pic,cad_data)
        '''z: torch.Size([1, 64, 256])'''
        res,loss_ita = self.forward_decoder(z,command, args)
        return res,loss_ita
