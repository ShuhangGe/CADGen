...
#         commands = nn.softmax(out_logits[0],dim=-1, keepdim=True)
#         paramaters = nn.softmax(out_logits[1],dim=-1)
#         '''command: [50, 60, 1]  paramaters: [50, 60, 16]'''
#         allcommand = torch.cat((command, paramaters),dim=-1)
#         '''allcommand: [50, 60, 17]'''

#         print('command.shape: ',commands.shape)
#         print('paramaters.shape: ',paramaters.shape)
#         print('allcommand.shape: ',allcommand.shape)
#         one_command = allcommand[0,:,:]
#         one_command = one_command.cpu().detach().numpy()
#         #save one_command to one_command.txt
#         np.savetxt('/scratch/sg7484/CADGen/bulletpoints/mae_cad/one_command.txt',one_command)
