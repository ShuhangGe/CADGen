...
out_paths = sorted(glob.glob(os.path.join(src_dir, "*._out_vec.txt")))
if args.num != -1:
...
for path in out_paths:
    ...
    try:
        ...
        #         print('out_vec1111111111111')
        #         out_vec = fp["out_vec"][:].astype(np.float)
        #         print('out_vec222222222222222')
        ...
        out_vec = np.loadtxt(path)
        out_vec = out_vec[:].astype(np.float)
        print('out_vec222222222222222')
        out_shape = vec2CADsolid(out_vec)
        if args.with_gt:
            gt_vec = fp["gt_vec"][:].astype(np.float)
    ...
    if args.form == "h5" and args.with_gt:
        ...
        sig = inspect.signature(local_display.DisplayShape)
        for name,param in sig.parameters.items():
            print(name, param.default)
        local_display.DisplayShape(out_shape, update=True, dump_image_path='/home/shuhang/Desktop/CADGen/result', dump_image_filename='1.png')
        # local_display.DisplayShape(out_shape, update=True)
        print(cnt)
    cnt += 1
