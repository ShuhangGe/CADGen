...
class Extrude(object):
    ...
    def from_dict(all_stat, extrude_id, sketch_dim=256):
        ...
        #print('extrude_id: ', extrude_id)
        print('extrude_entity: ',extrude_entity)
        ...
        for i in range(len(extrude_entity["profiles"])):
            #get sketch information from extrude
            sket_id, profile_id = extrude_entity["profiles"][i]["sketch"], extrude_entity["profiles"][i]["profile"]
            print('sket_id, profile_id: ',sket_id, profile_id)
            #from whole data get sketch_entity
            sket_entity = all_stat["entities"][sket_id]
            print('sket_entity: ',sket_entity)
            sket_profile = Profile.from_dict(sket_entity["profiles"][profile_id])# "JGC"
            print('sket_profile: ', sket_profile)
            sket_plane = CoordSystem.from_dict(sket_entity["transform"])
            print('sket_plane: ', sket_plane)
            # normalize profile
    ...
    def numericalize(self, n=256):
        ...
        self.extent_one = round(((self.extent_one + 1.0) / 2 * n))#.clip(min=0, max=n-1).astype(np.int)
        self.extent_two = round(((self.extent_two + 1.0) / 2 * n))#.clip(min=0, max=n-1).astype(np.int)
        ...
        self.sketch_size = round((self.sketch_size / 2 * n))#.clip(min=0, max=n-1).astype(np.int)
