...
class BertEncoderAsDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        #print('zero_negative_infinity: ',zero_negative_infinity)#all zeros
        #print('zero_negative_infinity.shape: ',zero_negative_infinity.shape)
        '''zero_negative_infinity.shape:  torch.Size([2, 101]) all zeros'''
        ...
        #print('tgt_bi_valid_mask: ',tgt_bi_valid_mask)
        ...
        if encoder_history_states is None:
            #print('encoder_history_states is None')#yesyesyes
            result = self.encoder(
            ...
            '''print('result[0].shape: ',result[0].shape)
                result[0].shape:  torch.Size([2, 114, 768])'''
            
            #print('hidden_states.shape: s',hidden_states.shape)
            '''hidden_states.shape:  torch.Size([2, 114, 768])'''
            result = list(result)
            result[0] = result[0][:, num_memory:].transpose(0, 1)
            #print('result[0].shape: ',result[0].shape)
            '''result[0].shape:  torch.Size([13, 2, 768])'''
...
class TransformerDecoderTextualHead(TextualHead):
    ...
    def __init__(
        ...
        if visual_feature_size is not None:
            self.visual_projection = create_projecton_layer(
                visual_projection_type, visual_feature_size, self.textual_feature_size)#textual_feature_size:hidden_size
        else:
    ...
    def forward(
        ...
        hidden_states.shape:  torch.Size([2, 101, 768])
        caption_tokens.shape:  torch.Size([2, 13])
        '''
        #print('hidden_valid_mask.shape: ',hidden_valid_mask.shape)            # None
        #print('bi_valid_mask_caption.shape: ',bi_valid_mask_caption.shape)    # None
        #print('encoder_history_states.shape: ',encoder_history_states.shape)  # None
        if return_dict:
            ret = {}

        projected_visual_features = self.visual_projection(hidden_states) if hidden_states is not None else None
        '''print('projected_visual_features.shape: ',projected_visual_features.shape)
            projected_visual_features.shape:  torch.Size([2, 101, 768])'''
        if return_dict:
        ...
        #print('caption_embeddings.shape: ',caption_embeddings.shape)
        '''caption_embeddings.shape:  torch.Size([2, 13, 768])'''
        ...
        #print('uni_mask_zero_neg: ',uni_mask_zero_neg) #an Upper Triangular Matrix
        #print('uni_mask_zero_neg.shape: ',uni_mask_zero_neg.shape)
        '''uni_mask_zero_neg.shape:  torch.Size([13, 13])'''
        ...
        #print('caption_embeddings.shape: ',caption_embeddings.shape)
        '''caption_embeddings.shape:  torch.Size([13, 2, 768])'''
        if projected_visual_features is not None:
            #print('projected_visual_features is not None')
            '''projected_visual_features is not None'''
            projected_visual_features = projected_visual_features.transpose(0, 1)
        else:
            #print('projected_visual_features is None')
        ...
        #print('projected_visual_features.shape: ',projected_visual_features.shape)
        '''projected_visual_features.shape:  torch.Size([101, 2, 768])
        projected_visual_features: image features   caption_embeddings: text features'''
...
class CaptioningModel(nn.Module):
    ...
    def forward_one(self, batch, return_info=False):
        ...
        if 'conte xt' in batch:
            ...
            #print('visual_features.shape: ',visual_features.shape)
            #print('visual_features_valid.shape: ',visual_features_valid.shape)
    ...
    def forward_one_scst(self, batch, visual_features, visual_features_valid):
        ...
        #print('forward_one_scst')# not implement
        self.eval()
    ...
    def forward_one_ce(self, batch, visual_features, visual_features_valid, return_info):
        ...
        #print('forward_one_ce')########yesyesyes
        ...
        if self.training:
            ...
            '''caption_token_input: text features 
            visual_features: image features '''
            #print('caption_token_input: ',caption_token_input)
    ...
    def infer(self, batch, visual_features, visual_features_valid,
              search_param=None):
        '''visual_features: image features'''
        #print('visual_features_valid: ',visual_features_valid)
        '''visual_features_valid:  None'''
...
class GeneratorWithBeamSearch(object):
    ...
    def search(
        ...
        print('GeneratorWithBeamSearch_search \n')
        if num_return_sequences != 1:
        ...
        print('batch_size: ',batch_size, 'cur_len: ',cur_len)
        print('num_beams: ',num_beams)
        print('pad_token_id: ',pad_token_id)
        print('eos_token_ids: ',eos_token_ids)
        print('per_node_beam_size: ',per_node_beam_size)
        print('repetition_penalty: ',repetition_penalty)
        print('temperature: ',temperature)
        '''batch_size:  1 cur_len:  1
            num_beams:  4
            pad_token_id:  102
            eos_token_ids:  [102]
            ...
            input_ids.shape1:  torch.Size([4, 1])'''
        ...
        while cur_len < max_length:
            #print('step: ',step)
            scores = step(input_ids)  # (batch_size * num_beams, cur_len, vocab_size) encoder text
            #print('scores.shape',scores.shape)
            '''scores.shape torch.Size([4, 30522])'''
