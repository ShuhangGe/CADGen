...
class Views2Points(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.text_proj = nn.Linear(config.dim_z, config.momentum_dim)
        self.image_proj = nn.Linear(config.UNet_out, config.momentum_dim)
        # get momentum features
        self.img_feature_m = ResNet50(resnet_out = int(config.resnet_out/4))             
        self.unet_m = ResidualUNet3D(config.resnet_out*3,config.UNet_out,num_levels = 3)
        self.text_encoder_m = Text_Encoder(config)
        self.text_proj_m = nn.Linear(config.dim_z, config.momentum_dim)
        self.image_proj_m = nn.Linear(config.UNet_out, config.momentum_dim)
        
        self.model_pairs = [[self.img_feature,self.img_feature_m],
                            [self.unet,self.unet_m],
        ...
        self.copy_params()
        self.register_buffer("image_queue", torch.randn(config.UNet_out, config.queue_size))
        self.register_buffer("text_queue", torch.randn(config.dim_z, config.queue_size))
    ...
    def encoder(self, side,front,top,cad_data):
    ...
    def decoder(self, image_feature, command, paramaters, do_momentum=True,alpha=0.4):
        #attention mask
        ...
        future_mask = _generate_future_mask(text_feature.shape[1],text_feature.dtype,text_feature.device)
        ...
        if do_momentum:
            # get momentum features
            image_feat = F.normalize(self.image_proj(torch.max(image_feature, dim=1, keepdim=False)[0]))  
            print('image_feat.shape: ',image_feat.shape)
            '''image_feat.shape:  torch.Size([10, 256])'''
            text_feat = F.normalize(self.text_proj(text_feature[:,0,:]),dim=-1)
            print(text_feat.shape)
            '''torch.Size([10, 256])'''
            with torch.no_grad():
                self._momentum_update()
                image_feature_m = image_feature.clone()

                image_feat_m = F.normalize(self.image_proj_m(torch.max(image_feature_m, dim=1, keepdim=False)[0]))
                image_feat_all = torch.cat([image_feat_m.t(),self.image_queue.clone().detach()],dim=1) 
                text_m = self.text_encoder_m(command, paramaters)
                text_feature_m = text_m.permute(1,0,2)
                text_feat_m = F.normalize(self.text_proj(text_feature_m[:,0,:]),dim=-1)
                ...
                #print('sim_i2t_targets.shape: ',sim_i2t_targets.shape) 
                sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_targets        

            sim_i2t = image_feat @ text_feat_all / self.temp
            #print('sim_i2t.shape: ',sim_i2t.shape) 
            sim_t2i = text_feat @ image_feat_all / self.temp
            #print('sim_t2i.shape: ',sim_t2i.shape) 
            '''sim_i2t.shape:  torch.Size([4, 57604])
            sim_t2i.shape:  torch.Size([4, 57604])'''
            loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=1)*sim_i2t_targets,dim=1).mean()
            #print('loss_i2t.shape: ',loss_i2t.shape) 
            loss_t2i = -torch.sum(F.log_softmax(sim_t2i, dim=1)*sim_t2i_targets,dim=1).mean() 
            #print('loss_t2i.shape: ',loss_t2i.shape) 
        ...
        #decoder input: [tgt_len,batch_size, embed_dim]
        ...
        return res,loss_ita
    def forward(self,side,front,top,cad_data,command,paramaters,do_momentum=True):
        with torch.no_grad():
            self.temp.clamp_(0.001,0.5)
        image_feature = self.encoder(side,front,top,cad_data)
        print('image_feature.shape: ',image_feature.shape)
        '''image_feature.shape:  torch.Size([10, 512, 256])'''
        if do_momentum:
            res, loss_ita = self.decoder(image_feature, command, paramaters, do_momentum, self.alpha)
            return res,loss_ita
        else:
            res, _ = self.decoder(image_feature, command, paramaters, do_momentum, self.alpha)
            return res
