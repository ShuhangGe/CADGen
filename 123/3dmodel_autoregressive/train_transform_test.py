...
from model.model_transformer_test import Views2Points
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for index, data in enumerate(train_loader,0):
            ...
            with autocast():
                output,loss_ita = model(front_pic,top_pic,side_pic,cad_data,train_command,train_paramaters,do_momentum=True)
                #print('output: ',output)
        ...
        with torch.no_grad():
            for index, data in enumerate(test_loader,0):
                ...
                #print('---------------')
                with autocast():
                    output_test = model(front_pic,top_pic,side_pic,cad_data,train_command,train_paramaters,do_momentum=False)
                    #print('output: ',output)
