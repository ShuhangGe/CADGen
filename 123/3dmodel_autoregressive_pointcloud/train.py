...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for index, data in enumerate(train_loader,0):
            ...
            with autocast():
                output,loss_ita = model(front_pic,top_pic,side_pic,cad_data,train_command,train_paramaters)
                #print('output: ',output)
            ...
            loss = sum(loss_dict.values())+loss_ita
            print('loss: ',loss)
        ...
        writer.add_scalar(f'loss_cmd_train_{args.lr}_', loss_cmd_train, global_step=epoch)
        writer.add_scalar(f'loss_args_train', loss_args_train, global_step=epoch)
        writer.add_scalar(f'loss_sum', loss_sum, global_step=epoch)
        ...
        with torch.no_grad():
            for index, data in enumerate(test_loader,0):
                ...
                #print('---------------')
                with autocast():
                    output_test,loss_ita = model(front_pic,top_pic,side_pic,cad_data,train_command,train_paramaters)
                    #print('output: ',output)
                    ...
                    loss_test = sum(loss_dict_test.values())+loss_ita
                #with autocast():
        ...
        writer.add_scalar(f'loss_cmd_test', loss_cmd_test, global_step=epoch)
        writer.add_scalar(f'loss_args_test', loss_args_test, global_step=epoch)
        writer.add_scalar(f'loss_test_sum', loss_test_sum, global_step=epoch)
