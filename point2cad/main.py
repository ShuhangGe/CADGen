...
def main():
    ...
    train_data = Point2CAD_dataset(args,test=False)
    test_data = Point2CAD_dataset(args,test=True)
    ...
    for epoch in range(args.epochs):
        ...
        for index, data in enumerate(train_loader,0):
            ...
            '''command.shape:  torch.Size([50, 60])
            paramaters.shape:  torch.Size([50, 60, 16])
            cad_data.shape:  torch.Size([50, 1024, 3])'''
            print('cad_data: ',cad_data)
